...
- `# PROCESS` — **現 IS（手段）を実現する進捗**を phase / 条件で分け、各行 `[done]` / `[todo]`（進捗はこの mark が担う。並行する手段ではない）。**`[done]` = その手段が実装され存在する事実**（「実装されたか」だけを見る ＝ あなたの call で flip してよく operator 確認は要らない）。**効果・満足の評価は別レイヤで、それは `state:done` ＝ operator の解決 act が担う**（解決の act が、実装済の PROCESS を「満足」・未実装の PROCESS を「諦め」と仕分ける。解決／未解決は可逆に設定できる）。
