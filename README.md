...
- **練習モード（音ゲー型）**: 落ちてくるノーツを判定ラインで叩く。ゲームに触れず自分で弾いて練習
...
**「🎮 練習モード」** ボタンで、落ちてくるノーツを判定ラインで叩く音ゲー風の練習ができます。
**ゲーム本体には一切キーを送らない自己完結トレーナー**なので、安全に指の練習ができます。
...
- **この練習ウィンドウにフォーカス**して、ノーツが判定ラインに来た瞬間に対応キーを押します
- **Perfect / Good / Miss** で判定し、**コンボ・スコア・精度**を表示。終了時にリザルト
- 内蔵の**音声を同期再生**（見て・聴いて・叩く）。「音を鳴らす」でオン/オフ
- **速度**（0.5〜1.25倍）を落として、ゆっくり練習できます
- Space または「スタート」で開始／もう一度

> レーンは「その曲で使うキー」だけを音高順に並べます。ここで指が覚えれば、
> そのままゲーム内の手弾きにも活かせます。
