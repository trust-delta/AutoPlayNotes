...
**🎮 練習 / 🎹 演奏 / 🎵 プレイリスト / ⚙ 設定 / 📄 ログ** のタブ構成で、右上のスイッチで**ライト / ダーク**を切り替えられます（設定は保存されます）。
起動時は **「🎮 練習」タブ**から開きます（前回開いていたタブがあれば、そちらを覚えています）。
...
- **モダンなタブ構成 UI**（customtkinter・練習 / 演奏 / プレイリスト / 設定 / ログ）と、**ライト / ダーク**テーマ切替（設定保存・タイトルバーも追従）
...
曲が最後まで鳴り終わると、**「この曲、弾いてみますか？」** という案内が一度だけ出ます。
そのまま「🎮 弾いてみる」で演奏範囲の選択 → 練習モードへ進めます。

案内は**同じ曲につき 1 回**で、途中で停止したとき・プレイリスト再生中・補助演奏のときは出ません。
不要なら案内の「今後は表示しない」か、**「🎮 練習」タブの「自動演奏が終わったら、練習に誘う」**で切り替えられます。
