...
一度メロディだけで読み込むと原曲へ戻すには読み直すしかない。ここでは難易度を
...
- `top`: 高い音から残す（メロディ抽出）
- `outer`: 最高音（メロディ）と最低音（根音）を優先し、余りは高い方から埋める
"""
...
LADDER: tuple[Level, ...] = (
    ...
    Level(
        index=2,
        name="メロディだけ・原速",
        description="同じ音を、曲の速さで。",
        max_notes=1,
    ),
    Level(
        index=3,
        name="メロディ＋根音",
        description="和音の一番低い音を足します。左手が入ります。",
        max_notes=2,
        strategy="outer",
    ),
    Level(
